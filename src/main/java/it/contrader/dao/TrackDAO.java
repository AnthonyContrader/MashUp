...
public class TrackDAO implements DAO<Track>{
	...
	public boolean insert(Track tracktoInsert) {
		...
		Connection connection = ConnectionSingleton.getInstance();
		try {
			PreparedStatement prepareStatement = connection.prepareStatement(QUERY_CREATE);
			prepareStatement.setString(1, tracktoInsert.getTitle());
			prepareStatement.setString(2, tracktoInsert.getAuthor() );
